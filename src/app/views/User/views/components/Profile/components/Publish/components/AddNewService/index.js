...
class AddNewService extends Component {
  ...
  addTitle(e) {
    ...
    title !== "" && title.length >= 4 ? AddTitle(title) : AddTitle("");
  }
  deleteTag(e) {
  ...
  newService() {
    ...
    const { addNewService, ChangeEditingState, CreateNewService, newServiceConfig, tags } = this.props;
        ...
        <div className={styles.tags}>
          ...
          <div className={styles.tagList}>
            <ul className={styles.list}>
              {tags.map((tag, i) => (
                <li className={styles.item} key={i} onClick={deleteTag}>{`#${
                  tag
                }`}</li>
              ))}
            </ul>
